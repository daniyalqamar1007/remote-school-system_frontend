...
export default function DepartmentsPage() {
  ...
  const handlePageChange = (page: number) => {
    setCurrentPage(page)
    fetchDepartments(page, pageSize, searchTerm)
  }

  const handlePageSizeChange = (newSize: number) => {
    setPageSize(newSize)
    setCurrentPage(1)
    fetchDepartments(1, newSize, searchTerm)
  }
  ...
  const handleCreateDepartment = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token') || localStorage.getItem('accessToken')
      await axios.post(`${process.env.NEXT_PUBLIC_SRS_SERVER}/admin/departments`, formData, {
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const handleEditDepartment = async () => {
    ...
    try {
      setIsSubmitting(true)
      const token = localStorage.getItem('token') || localStorage.getItem('accessToken')
      await axios.put(`${process.env.NEXT_PUBLIC_SRS_SERVER}/admin/departments/${departmentToEdit?._id}`, formData, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
                    ...
                    <TableRow key={d._id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="sm" title="View" onClick={() => router.push(`/secretary/departments/view/${d._id}`)}><Eye className="h-4 w-4" /></Button>
      ...
      <Dialog open={isModalOpen} onOpenChange={(open) => {
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateDepartment} disabled={isSubmitting || !formData.departmentName.trim()}>
              {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : "Create Department"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleEditDepartment} disabled={isSubmitting || !formData.departmentName.trim()}>
              {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : "Update Department"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
