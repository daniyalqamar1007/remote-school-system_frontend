...
export default function ClubAssignmentsPage() {
  ...
  const normalizeStudentsResponse = (data: any) => {
    if (!data) return []
  ...
  const fetchStudents = async () => {
    try {
      const response = await adminApi.students.getAll({ page: '1', limit: '500' })
      const studentsArray = normalizeStudentsResponse(response)
  ...
  const fetchMemberships = useCallback(async () => {
    ...
    const targetClubs = filterClub === 'all'
      ? clubs
      : clubs.filter((club: any) => club._id === filterClub)
    ...
    const clubLookup = new Map(targetClubs.map((club: any) => [club._id?.toString(), club]))
    ...
    try {
      ...
      const membershipPromises = targetClubs.map(async (club: any) => {
        try {
          const response = await fetch(`${apiBase}/clubs/${club._id}/members?${params.toString()}`, {
            headers
          })
          if (!response.ok) {
            throw new Error(`Failed to fetch members for ${club.name}`)
          }
          const data = await response.json()
      ...
      const allMemberships = await Promise.all(membershipPromises)
      const normalizedMemberships = allMemberships.flat().map((membership: any) => {
        if (membership.clubId && membership.clubId.name) {
          return membership
        }
        const membershipClubId = typeof membership.clubId === 'object' && membership.clubId !== null
          ? membership.clubId._id
          : membership.clubId
        const fallbackClub = clubLookup.get(membershipClubId?.toString())

        return {
          ...membership,
          clubId: fallbackClub
            ? { _id: fallbackClub._id, name: fallbackClub.name, type: fallbackClub.type }
            : membership.clubId
        }
      })
  ...
  }, [apiBase, clubs, debouncedSearch, filterClub, roleFilter])
  ...
  const fetchPendingRequests = useCallback(async () => {
    ...
    try {
      ...
      const requestPromises = targetClubs.map(async (club: any) => {
        try {
          const response = await fetch(`${apiBase}/clubs/${club._id}/membership/pending?${params.toString()}`, {
            headers
          })
          if (!response.ok) {
            throw new Error(`Failed to fetch pending requests for ${club.name}`)
          }
          const data = await response.json()
          return Array.isArray(data) ? data : []
        } catch (error) {
      ...
      const normalizedRequests = allRequests.flat().map((request: any) => {
        if (request.clubId && request.clubId.name) {
          return request
        }
        const requestClubId = typeof request.clubId === 'object' && request.clubId !== null
          ? request.clubId._id
          : request.clubId
        const fallbackClub = clubLookup.get(requestClubId?.toString())
  ...
  }, [apiBase, clubs, debouncedSearch, filterClub, requestRoleFilter])

  useEffect(() => {
    const initialize = async () => {
      try {
        await Promise.all([fetchStudents(), fetchClubs()])
      } finally {
  ...
  }, [clubs, fetchMemberships, fetchPendingRequests])
  ...
  const handleAssignStudent = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Auto-approve the membership since it's secretary-created
                  ...
                  <TableBody>
                    {paginatedMemberships.map((membership: any) => (
                      <TableRow key={membership._id}>
                        ...
                        <TableCell>{membership.clubId.name}</TableCell>
                        <TableCell>
                          <Badge className={getTypeColor(membership.clubId.type)}>
                            {membership.clubId.type}
                          </Badge>
                        </TableCell>
                        <TableCell>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="flex flex-wrap gap-4 mb-4">
                ...
                <Select value={requestRoleFilter} onValueChange={setRequestRoleFilter}>
                  <SelectTrigger className="w-[200px]">
                    <SelectValue placeholder="Filter by role" />
                  </SelectTrigger>
                  <SelectContent>
                  ...
                  <TableBody>
                    {paginatedRequests.map((request: any) => (
                      <TableRow key={request._id}>
                        ...
                        <TableCell>{request.clubId.name}</TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <Button
                            size="sm"
                            onClick={() => handleApproveRequest(request._id, request.clubId._id)}
                          >
                            Approve
                          </Button>
                        </TableCell>
