...
export default function ManageParentsPage() {
  ...
  const fetchParents = async (page = 1, limit = 5, search = '', gender = 'all', parentType = 'all') => {
    try {
      setLoading(true)
  ...
  const handlePageChange = (newPage: number) => {
    ...
    fetchParents(newPage, pageSize, searchTerm, genderFilter, parentTypeFilter)
  }

  const handlePageSizeChange = (newSize: number) => {
    setPageSize(newSize)
    setCurrentPage(1)
    fetchParents(1, newSize, searchTerm, genderFilter, parentTypeFilter)
  }

  // Handle search with debounce
  const debouncedSearch = useCallback(
    debounce((search: string, gender: string, parentType: string) => {
      setCurrentPage(1)
      fetchParents(1, pageSize, search, gender, parentType)
    }, 500),
    [pageSize]
  )
  ...
  const handleSearchChange = (value: string) => {
    setSearchTerm(value)
    debouncedSearch(value, genderFilter, parentTypeFilter)
  }
  ...
  const handleGenderFilterChange = (value: string) => {
    ...
    setCurrentPage(1)
    fetchParents(1, pageSize, searchTerm, value, parentTypeFilter)
  }
  ...
  const handleParentTypeFilterChange = (value: string) => {
    ...
    setCurrentPage(1)
    fetchParents(1, pageSize, searchTerm, genderFilter, value)
  }
  ...
  const handleAddParentSuccess = () => {
    ...
    fetchParents(currentPage, pageSize, searchTerm, genderFilter, parentTypeFilter)
  }
  ...
  const handleViewParent = (parent: Parent) => {
    router.push(`/secretary/manage-parents/view/${parent._id}`)
  }
