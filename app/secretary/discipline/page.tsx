...
import { AlertTriangle, Calendar, Clock, MapPin, FileText, Bell, CheckCircle, XCircle, Loader2, Play, X, Mail, Phone, FileCheck, Trash2 } from 'lucide-react';
...
const DisciplinePage = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Discipline stats response:', data);
        setStats({
          total: data.total || 0,
  ...
  const fetchDisciplinaryActions = async (page: number = 1) => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('Disciplinary actions response:', data);
        ...
        console.log('Processed actions:', actionsList.length, actionsList);
  ...
  const fetchStudents = async () => {
    try {
      ...
      while (hasMore) {
        const response = await fetch(`${process.env.NEXT_PUBLIC_SRS_SERVER}/admin/students?page=${page}&limit=100`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
  ...
  const filteredActions = actions.filter(action => {
    const student = action.student as { firstName?: string; lastName?: string; studentId?: string } || {};
    const matchesSearch = 
