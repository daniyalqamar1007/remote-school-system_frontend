...
export default function NewProgramPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!user?.schoolId) {
        toast.error('No school ID is assigned to your secretary account. Please contact the system administrator.')
        setLoading(false)
        return
      }
      ...
      if (response?.success || response?.statusCode === 201 || response?.data || response?._id) {
        ...
        router.push('/secretary/sports?refresh=true')
      } else {
