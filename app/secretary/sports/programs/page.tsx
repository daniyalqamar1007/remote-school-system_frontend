...
export default function SportsProgramsPage() {
  ...
  const handleViewProgram = (program: any) => {
    router.push(`/secretary/sports/programs/${program._id}`)
  }

  const handleEditProgram = (program: any) => {
    router.push(`/secretary/sports/programs/${program._id}/edit`)
  }
  ...
  const confirmDeleteProgram = async () => {
    ...
    try {
      ...
      // Accept both boolean and object return types
      if (
        (typeof result === "object" && result !== null && (result as any).success) ||
        result === true ||
        (typeof result === "object" && result !== null && (result as any).statusCode === 200)
      ) {
        toast.success(
          (typeof result === "object" && result !== null && (result as any).message)
            ? (result as any).message
            : 'Sports program deleted successfully'
        );
      } else {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <Link href="/secretary/sports/programs/new">
