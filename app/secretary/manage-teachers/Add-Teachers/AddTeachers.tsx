...
import { CheckIcon, Loader2, Eye, EyeOff, Camera, User, X, RefreshCw } from "lucide-react"
...
export default function AddTeacherModal({ isOpen, onClose, teacherData, onSuccess }: AddTeacherModalProps) {
  ...
  const generateEmployeeId = () => {
    if (!formData.firstName?.trim() || !formData.lastName?.trim()) {
      toast.error("Please enter first and last name before generating employee ID");
      return;
    }
    
    // Generate unique employee ID using timestamp + initials + random
    ...
    // Format: EMP + Initials + Timestamp + Random (e.g., EMPJD12345678901)
    const employeeId = `EMP${firstInitial}${lastInitial}${timestamp}${randomNum}`;
    
    setFormData({ ...formData, employeeId });
    
    // Clear any previous employeeId error
    if (errors.employeeId) {
      setErrors((prev: any) => ({ ...prev, employeeId: "" }));
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    id="employeeId"
                    placeholder="1000 or Generate ID"
                    value={formData.employeeId}
                    onChange={handleChange}
                    className={`flex-1 border-gray-300 focus:border-black focus:ring-black ${errors.employeeId ? 'border-red-600' : ''} ${teacherData ? 'bg-gray-100 cursor-not-allowed' : ''}`}
                    disabled={!!teacherData}
                  />
                  {!teacherData && (
                    <Button
