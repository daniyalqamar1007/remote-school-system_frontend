...
function TeacherViewContent() {
  ...
  const fetchTeacher = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${process.env.NEXT_PUBLIC_SRS_SERVER}/admin/teachers/${id}?schoolId=${schoolId}`, {
  ...
  useEffect(() => { if (id && schoolId) fetchTeacher() }, [id, schoolId])

  return (
    <div className="space-y-6 p-4 sm:p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Button variant="outline" onClick={() => router.push('/super-admin/manage/teachers')}><ArrowLeft className="h-4 w-4 mr-2" />Back</Button>
                    ...
                    {Array.isArray(teacher.courseAssignments) && teacher.courseAssignments.length > 0 ? (
                      teacher.courseAssignments.map((a: any) => (
                        <TableRow key={a._id}>
                          ...
                          <TableCell>{Array.isArray(a.grades) ? a.grades.map((g: any) => `${g.level}${g.section ? '-' + g.section : ''}`).join(', ') : '—'}</TableCell>
                        </TableRow>
                      ))
                    ) : (
