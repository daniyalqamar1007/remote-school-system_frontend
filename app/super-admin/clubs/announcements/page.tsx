...
const ClubAnnouncementsPage = () => {
  ...
  useEffect(() => {
    if (selectedSchoolId === 'all') {
      // Don't fetch clubs when "All Schools" is selected
      setClubs([]);
      setIsLoading(false);
    } else {
      fetchClubs();
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedSchoolId, selectedClub, statusFilter, debouncedSearchTerm]);
  ...
  const fetchAnnouncements = useCallback(async () => {
    try {
      ...
      // Add schoolId filter if a school is selected (not 'all')
  ...
  }, [debouncedSearchTerm, selectedClub, selectedSchoolId, statusFilter, currentPage, pageSize]);
  ...
  const fetchClubs = async (schoolId?: string) => {
    try {
      ...
      // Only add schoolId if a school is selected (not 'all')
      ...
      if (response.ok) {
        const data = await response.json();
        // Handle paginated response - extract clubs array
  ...
  // Fetch clubs for create dialog based on selected school
  const fetchClubsForCreateDialog = async (schoolId: string) => {
  ...
  const handleCreateAnnouncement = async () => {
    if (!createDialogSchoolId || createDialogSchoolId === '') {
    ...
    try {
      ...
      // Don't send createdBy - backend will set it from req.user
      const announcementData = {
        ...
        schoolId: createDialogSchoolId,
        clubId: newAnnouncement.clubId
      };

      const res = await fetch(`${process.env.NEXT_PUBLIC_SRS_SERVER}/admin/clubs/announcements`, {
        method: 'POST',
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    setNewAnnouncement({ ...newAnnouncement, clubId: '' });
                    fetchClubsForCreateDialog(value);
                  }}
                >
                  <SelectTrigger className={!createDialogSchoolId ? 'border-red-500' : ''}>
                ...
                {!createDialogSchoolId && <p className="text-sm text-red-600">Please select a school first</p>}
              </div>

              <div className="space-y-2">
                <Label htmlFor="clubSelect">
                  Select Club <span className="text-red-600">*</span>
                </Label>
                <Select
                  value={newAnnouncement.clubId}
                  onValueChange={(value) => setNewAnnouncement({ ...newAnnouncement, clubId: value })}
                  disabled={!createDialogSchoolId}
                >
                  <SelectTrigger className={formErrors.clubId ? 'border-red-500' : ''} disabled={!createDialogSchoolId}>
                    <SelectValue placeholder={createDialogSchoolId ? "Choose a club" : "Please select a school first"} />
                  </SelectTrigger>
                  <SelectContent>
                    {clubs.map(club => (
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex gap-4 mb-4">
            ...
            <Select value={selectedSchoolId} onValueChange={(value) => {
            ...
            >
              <SelectTrigger className="w-[200px]" disabled={!selectedSchoolId || selectedSchoolId === 'all'}>
                <SelectValue placeholder={selectedSchoolId && selectedSchoolId !== 'all' ? "Filter by club" : "Select school first"} />
              </SelectTrigger>
              <SelectContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    {announcements.map((announcement) => {
                      // Get school name from announcement.schoolId or announcement.clubId.schoolId
                      const schoolName = typeof announcement.schoolId === 'object' && announcement.schoolId?.name 
                        ? announcement.schoolId.name 
                        : typeof announcement.clubId?.schoolId === 'object' && announcement.clubId?.schoolId?.name
                        ? announcement.clubId.schoolId.name
                        : 'N/A';
                ...
                <div className="space-y-3 mt-2">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg space-y-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span>{announcementToSend.title}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold">Club:</span>
                      <span>{announcementToSend.clubId?.name || 'N/A'}</span>
                    </div>
                    {(() => {
                      const schoolName = typeof announcementToSend.schoolId === 'object' && announcementToSend.schoolId?.name 
                        ? announcementToSend.schoolId.name 
                        : typeof announcementToSend.clubId?.schoolId === 'object' && announcementToSend.clubId?.schoolId?.name
                        ? announcementToSend.clubId.schoolId.name
                        : 'N/A';
                      return (
                        <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className={getAudienceColor(announcementToSend.targetAudience || 'members')}>
                        {announcementToSend.targetAudience === 'members' ? 'Club Members' :
                         announcementToSend.targetAudience === 'parents' ? 'Parents Only' :
                         announcementToSend.targetAudience === 'both' ? 'Members & Parents' :
                         'N/A'}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold">Priority:</span>
                      <Badge className={getPriorityColor(announcementToSend.priority || 'normal')}>
                        {announcementToSend.priority || 'Normal'}
                      </Badge>
                    </div>
                  </div>
                  <p className="text-sm text-muted-foreground mt-2">
                    This announcement will be sent to all {announcementToSend.targetAudience === 'members' ? 'club members' :
                     announcementToSend.targetAudience === 'parents' ? 'parents' :
                     announcementToSend.targetAudience === 'both' ? 'club members and parents' :
                     'recipients'} of <strong>{announcementToSend.clubId?.name || 'the selected club'}</strong>.
                  </p>
                </div>
              ) : (
                'Are you sure you want to send this announcement now?'
