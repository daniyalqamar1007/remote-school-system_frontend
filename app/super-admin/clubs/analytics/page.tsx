...
const ClubAnalyticsPage = () => {
  ...
  const fetchAnalytics = async () => {
    if (!selectedSchoolId || selectedSchoolId === '') {
      setAnalytics(null);
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_SRS_SERVER}/admin/clubs/analytics?range=${timeRange}&schoolId=${selectedSchoolId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  if (error || (!analytics && selectedSchoolId)) {
    return (
      <div className="flex flex-col items-center justify-center h-screen space-y-4">
    ...
    return (
      <div className="container mx-auto px-4 py-6 space-y-6">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" onClick={() => router.push('/super-admin/clubs')}>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              <Label className="text-lg font-semibold">Please select a school to view analytics</Label>
