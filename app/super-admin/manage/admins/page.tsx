...
import AddAdminModal from "./Add-Admins/AddAdmins"
...
export default function AdminsTable() {
  ...
  const fetchAdmins = useCallback(async (page = 1, pageLimit = 10, query = "", schoolId = "") => {
    try {
      setLoading(true)
      const token = localStorage.getItem('token') || localStorage.getItem('accessToken') || localStorage.getItem('authToken')
  ...
  useEffect(() => {
    fetchAdmins(1, limit, "", selectedSchoolId)
  }, [fetchAdmins])
  ...
  useEffect(() => {
    fetchAdmins(1, limit, debouncedSearch, selectedSchoolId)
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`${process.env.NEXT_PUBLIC_SRS_SERVER}/super-admin/admins/${adminToDelete._id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        }
      ...
      fetchAdmins(currentPage, limit, debouncedSearch, selectedSchoolId)
    } catch (error) {
  ...
  const handleEditAdmin = async (admin: Admin) => {
    try {
      setLoading(true)
      const token = localStorage.getItem('token') || localStorage.getItem('accessToken') || localStorage.getItem('authToken')
      const res = await axios.get(`${process.env.NEXT_PUBLIC_SRS_SERVER}/super-admin/admins/${admin._id}`, {
        headers: { 'Authorization': `Bearer ${token}` },
      })
  ...
  const handlePageChange = (page: number) => {
    ...
    fetchAdmins(p, limit, debouncedSearch, selectedSchoolId)
  }
  ...
  const handleSchoolFilterChange = (schoolId: string) => {
    ...
    setCurrentPage(1)
    fetchAdmins(1, limit, debouncedSearch, next)
  }

  return (
    <div className="container mx-auto py-6 lg:py-10 px-4 lg:px-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h1 className="text-2xl lg:text-3xl font-bold">Admins</h1>
        ...
        <>
                    ...
                    <TableRow key={admin._id}>
                      <TableCell>
                        <div>
                          ...
                          <div className="text-sm text-gray-500 sm:hidden truncate">{admin.email || '—'}</div>
                          <div className="text-sm text-gray-500 md:hidden">{admin.schoolId?.name || '—'}</div>
                        </div>
                      </TableCell>
                      <TableCell className="max-w-[200px] truncate hidden sm:table-cell">{admin.email || '—'}</TableCell>
                      <TableCell className="hidden md:table-cell">{admin.schoolId?.name || '—'}</TableCell>
          ...
          <div className="flex items-center justify-between px-2 py-4">
            <div className="flex items-center gap-2">
              ...
              <select
                value={String(limit)}
                onChange={(e) => fetchAdmins(1, Number(e.target.value), debouncedSearch, selectedSchoolId)}
      ...
      <AddAdminModal
        ...
        onSuccess={() => fetchAdmins(1, limit, debouncedSearch, selectedSchoolId)}
      />
    </div>
  )
}
