...
export default function EditParentModal({ isOpen, onClose, onSuccess, parentId, schools }: EditParentModalProps) {
  ...
  const fetchParentData = async () => {
    ...
    try {
      ...
      if (response.data.success && response.data.data?.parent) {
        const parent: ParentData = response.data.data.parent
        ...
        // Get school name from belongToSchools array (first active school)
        let schoolName = ""
        let schoolId = ""
        if (parent.belongToSchools && parent.belongToSchools.length > 0) {
          const activeSchool = parent.belongToSchools.find(s => s.isActive)
          if (activeSchool) {
            schoolName = activeSchool.schoolName || ""
            schoolId = activeSchool.schoolId || ""
          } else {
            // If no active school, use first one
            schoolName = parent.belongToSchools[0].schoolName || ""
            schoolId = parent.belongToSchools[0].schoolId || ""
          }
        }
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2 w-full min-w-0">
                  ...
                  <Input
                    id="schoolId"
                    type="text"
                    value={selectedSchoolName || "No school assigned"}
                    disabled
                    className="w-full bg-gray-50 cursor-not-allowed"
                  />
                  <p className="text-xs text-gray-500">School cannot be changed</p>
                </div>
