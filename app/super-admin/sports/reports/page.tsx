...
export default function SportsReportsPage() {
    ...
    const fetchSchools = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (schoolsList.length > 0 && selectedSchoolId === 'all') {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const programsRes = await sportsApi.programs.getAll({ 
        page: '1', 
        limit: '100',
      ...
      // Fetch overview report with schoolId - all-time data (no date filters)
    ...
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 relative">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-sm text-gray-500 mt-2">Showing all-time data for selected school</p>
        </CardContent>
      </Card>
      ...
      {selectedSchoolId === 'all' && (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-gray-600">Please select a school to view reports</p>
          </CardContent>
        </Card>
      )}
    </div>
