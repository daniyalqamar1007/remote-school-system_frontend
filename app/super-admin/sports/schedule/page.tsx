...
export default function SportsSchedulePage() {
  ...
  useEffect(() => {
    fetchSchools()
    fetchPrograms()
    fetchSchedulesData(currentPage, pageSize, '', 'all', 'all', 'all', 'all')
  }, [])

  useEffect(() => {
    debouncedSearch(searchTerm, typeFilter, dateFilter, schoolFilter)
  }, [searchTerm, typeFilter, dateFilter, schoolFilter])

  // Handle search with debounce
  const debouncedSearch = useCallback(
    debounce((search: string, type: string, date: string, school: string) => {
      setCurrentPage(1)
      fetchSchedulesData(1, pageSize, search, 'all', type, date, school)
    }, 500),
    [pageSize]
  )
  ...
  const fetchSchedulesData = async (page: number, limit: number, search: string, program: string, type: string, date: string, school: string) => {
    try {
  ...
  const handlePageChange = (page: number) => {
    ...
    fetchSchedulesData(page, pageSize, searchTerm, 'all', typeFilter, dateFilter, schoolFilter)
  }

  const handlePageSizeChange = (newSize: number) => {
    setPageSize(newSize)
    setCurrentPage(1)
    fetchSchedulesData(1, newSize, searchTerm, 'all', typeFilter, dateFilter, schoolFilter)
  }
  ...
  const confirmDeleteSchedule = async () => {
    ...
    try {
      ...
      await fetchSchedulesData(currentPage, pageSize, searchTerm, 'all', typeFilter, dateFilter, schoolFilter)
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/super-admin/sports/programs">
        ...
        <Link href="/super-admin/sports/schedule/new">
              ...
              <TableBody>
                {schedulesLoading ? (
                  ...
                  schedules.map((schedule: any) => {
                    ...
                    const scheduleEndDate = schedule.endDate || schedule.startDate || schedule.date
                      ...
                      >
                        ...
                        <TableCell className="hidden sm:table-cell">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm">
                              {schedule.schoolId?.name || schedule.school?.name || program?.schoolId?.name || program?.school?.name || 'N/A'}
                            </span>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-1">
                            <Link href={`/super-admin/sports/schedule/${schedule._id}`}>
                              <Button variant="ghost" size="sm" title="View">
                                <Eye className="h-4 w-4 text-black" />
                              </Button>
                            </Link>
                            <Link href={`/super-admin/sports/schedule/${schedule._id}/edit`}>
