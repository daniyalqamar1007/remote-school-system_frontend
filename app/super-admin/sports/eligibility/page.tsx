...
export default function EligibilityMedicalPage() {
  ...
  useEffect(() => {
    fetchSchools()
    fetchPrograms()
    fetchAssignments(currentPage, pageSize, searchTerm, 'all', clearanceFilter, schoolFilter)
  }, [])

  useEffect(() => {
    // Debounce search
    const timer = setTimeout(() => {
      fetchAssignments(1, pageSize, searchTerm, 'all', clearanceFilter, schoolFilter)
      setCurrentPage(1)
    }, 300)
    return () => clearTimeout(timer)
  }, [searchTerm, clearanceFilter, schoolFilter, pageSize])
  ...
  const fetchAssignments = async (page: number, limit: number, search: string, _program: string, clearance: string, school: string) => {
    try {
      ...
      const filteredTotal = assignmentsList.length
      ...
      setTotalAssignments(filteredTotal)
    } catch (error: any) {
  ...
  const handleCheckEligibility = async (assignment: any) => {
    try {
      ...
      await fetchAssignments(currentPage, pageSize, searchTerm, 'all', clearanceFilter, schoolFilter)
    } catch (error: any) {
