...
export default function SportsAttendancePage() {
  ...
  useEffect(() => {
    debouncedSearch(searchTerm, 'all', statusFilter, schoolFilter)
  }, [searchTerm, statusFilter, schoolFilter])

  // Handle search with debounce
  const debouncedSearch = useCallback(
    debounce((search: string, _schedule: string, status: string, school: string) => {
      setCurrentPage(1)
      fetchAttendanceData(1, pageSize, search, 'all', status, school)
    }, 500),
    [pageSize]
  )

  useEffect(() => {
    fetchAttendanceData(currentPage, pageSize, searchTerm, 'all', statusFilter, schoolFilter)
  }, [currentPage])
  ...
  const fetchAttendanceData = async (page: number, limit: number, search: string, _schedule: string, status: string, school: string) => {
    try {
  ...
  const handlePageChange = (page: number) => {
    ...
    fetchAttendanceData(page, pageSize, searchTerm, 'all', statusFilter, schoolFilter)
  }

  const handlePageSizeChange = (newSize: number) => {
    setPageSize(newSize)
    setCurrentPage(1)
    fetchAttendanceData(1, newSize, searchTerm, 'all', statusFilter, schoolFilter)
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link href="/super-admin/sports/attendance/take">
                        ...
                        <TableCell className="hidden sm:table-cell">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-900">
                              {record.schoolId?.name || record.school?.name || program?.schoolId?.name || program?.school?.name || schedule?.schoolId?.name || schedule?.school?.name || 'N/A'}
                            </span>
                          </div>
                        </TableCell>
                        <TableCell>
