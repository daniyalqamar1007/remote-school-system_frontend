...
export default function SportsProgramsPage() {
  ...
  useEffect(() => {
    // Fetch programs first (used in table) - 'all' for school means fetch all programs
    fetchProgramsData(1, pageSize, '', 'all', 'all', 'all', 'all')
  ...
  const debouncedSearch = useCallback(
    debounce((search: string, season: string, type: string, isActive: string, school: string) => {
      setCurrentPage(1)
      fetchProgramsData(1, pageSize, search, season, type, isActive, school)
    }, 500),
    [pageSize]
  )
  ...
  const handleSearchChange = (value: string) => {
    setSearchTerm(value)
    debouncedSearch(value, seasonFilter, typeFilter, isActiveFilter, schoolFilter)
  }

  const handleSeasonFilterChange = (value: string) => {
    setSeasonFilter(value)
    setCurrentPage(1)
    fetchProgramsData(1, pageSize, searchTerm, value, typeFilter, isActiveFilter, schoolFilter)
  }

  const handleTypeFilterChange = (value: string) => {
    setTypeFilter(value)
    setCurrentPage(1)
    fetchProgramsData(1, pageSize, searchTerm, seasonFilter, value, isActiveFilter, schoolFilter)
  }

  const handleIsActiveFilterChange = (value: string) => {
    setIsActiveFilter(value)
    setCurrentPage(1)
    fetchProgramsData(1, pageSize, searchTerm, seasonFilter, typeFilter, value, schoolFilter)
  }

  const handleSchoolFilterChange = (value: string) => {
    setSchoolFilter(value)
    setCurrentPage(1)
    fetchProgramsData(1, pageSize, searchTerm, seasonFilter, typeFilter, isActiveFilter, value)
  }

  const handlePageChange = (page: number) => {
    setCurrentPage(page)
    fetchProgramsData(page, pageSize, searchTerm, seasonFilter, typeFilter, isActiveFilter, schoolFilter)
  }

  const handlePageSizeChange = (newSize: number) => {
    setPageSize(newSize)
    setCurrentPage(1)
    fetchProgramsData(1, newSize, searchTerm, seasonFilter, typeFilter, isActiveFilter, schoolFilter)
  }
  ...
  const fetchProgramsData = async (
    ...
    try {
      ...
      // Add schoolId for super-admin - only if a specific school is selected
      // If 'all' is selected, don't pass schoolId to get all programs from all schools
      if (school && school !== 'all') {
        params.schoolId = school
      }
      // If school is 'all', don't add schoolId param - backend will return all programs
  ...
  const handleViewProgram = (program: any) => {
    router.push(`/super-admin/sports/programs/${program._id}`)
  }

  const handleEditProgram = (program: any) => {
    router.push(`/super-admin/sports/programs/${program._id}/edit`)
  }
  ...
  const confirmDeleteProgram = async () => {
    ...
    try {
      ...
      fetchProgramsData(currentPage, pageSize, searchTerm, seasonFilter, typeFilter, isActiveFilter, schoolFilter)
      fetchSportsData()
    } catch (error: any) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <Link href="/super-admin/sports/programs/new">
