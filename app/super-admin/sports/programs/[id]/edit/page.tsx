...
export default function EditProgramPage() {
  ...
  const [programSchoolId, setProgramSchoolId] = useState<string>('') // Store program's schoolId
  ...
  const loadProgram = async () => {
    try {
      ...
      if (program) {
        ...
        // Extract schoolId from program (handle both populated and unpopulated)
        const schoolId = program.schoolId?._id || program.schoolId || ''
        setProgramSchoolId(schoolId)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response?.success || response?.statusCode === 200 || response?.data) {
        ...
        router.push(`/super-admin/sports/programs?refresh=true`)
      } else {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href={`/super-admin/sports/programs/${params.id}`}>
