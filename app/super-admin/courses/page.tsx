...
export default function CoursesPage() {
  ...
  const [selectedSchoolIdForForm, setSelectedSchoolIdForForm] = useState<string>("")
  ...
  const fetchCourses = async (query = "", page = 1, pageLimit = 10, departmentId: string = selectedDepartmentId, schoolId: string = selectedSchoolId) => {
    try {
      setIsLoading(true)
  ...
  useEffect(() => {
    ...
    fetchCourses(debouncedSearchQuery, 1, limit, selectedDepartmentId, selectedSchoolId)
    ...
    if (selectedSchoolId) url.searchParams.set("schoolId", selectedSchoolId)
    else url.searchParams.delete("schoolId")
    window.history.pushState({}, "", url)
  }, [debouncedSearchQuery, selectedSchoolId])
  ...
  // Fetch departments (optionally by school)
  const fetchDepartments = async (schoolIdParam?: string) => {
    try {
  ...
  useEffect(() => {
    ...
    const schoolParam = urlParams.get("schoolId") || ""
    ...
    fetchCourses(searchParam, pageParam, limitParam, departmentParam, schoolParam)
  ...
  const openModal = () => {
    resetForm()
    // Wait for school selection to fetch departments; clear any stale departments
    setDepartments([])
    if (schools.length === 0) {
      fetchSchools()
    }
  ...
  const openEditModalById = async (courseId: string) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token') || localStorage.getItem('accessToken')
      // Get schoolId from the course being edited or from filter
      const courseData = courses.find(c => c._id === courseId)
      const schoolIdParam = (courseData as any)?.schoolId?._id || selectedSchoolId
      if (!schoolIdParam) {
        toast.error("School ID is required to edit course")
        setIsModalOpen(false)
        setIsEditing(false)
        return
      }
      // Fetch departments for this course's school
      await fetchDepartments(schoolIdParam)
      const url = `${process.env.NEXT_PUBLIC_SRS_SERVER}/admin/courses/${courseId}?schoolId=${schoolIdParam}`
      ...
      setFormData({
        ...
        schoolId: course?.schoolId?._id || course?.schoolId || "",
      })
      setSelectedSchoolIdForForm(course?.schoolId?._id || course?.schoolId || "")
    } catch (e) {
  ...
  const openCopyModalById = async (courseId: string) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token') || localStorage.getItem('accessToken')
      // Get schoolId from the course being copied or from filter
      const courseData = courses.find(c => c._id === courseId)
      const schoolIdParam = (courseData as any)?.schoolId?._id || selectedSchoolId
      if (!schoolIdParam) {
        toast.error("School ID is required to copy course")
        setIsModalOpen(false)
        return
      }
      // Fetch departments for this course's school
  ...
  const handleDepartmentFilterChange = (deptId: string) => {
    ...
    setCurrentPage(1)
    fetchCourses(debouncedSearchQuery, 1, limit, next, selectedSchoolId)
  ...
  const handleSchoolFilterChange = (schoolId: string) => {
    ...
    setCurrentPage(1)
    fetchCourses(debouncedSearchQuery, 1, limit, selectedDepartmentId, next)

    const url = new URL(window.location.href)
    if (next) url.searchParams.set("schoolId", next)
    else url.searchParams.delete("schoolId")
    url.searchParams.set("page", "1")
    url.searchParams.set("limit", String(limit))
    if (debouncedSearchQuery) url.searchParams.set("search", debouncedSearchQuery)
  ...
  const handlePageChange = (newPage: number) => {
    ...
    setCurrentPage(page)
    fetchCourses(debouncedSearchQuery, page, limit, selectedDepartmentId, selectedSchoolId)
  ...
  const handleDeleteCourse = async () => {
    ...
    try {
      setIsDeleting(true)
      const token = localStorage.getItem('token') || localStorage.getItem('accessToken')
      const schoolIdParam = (courseToDelete as any)?.schoolId?._id || selectedSchoolId
      if (!schoolIdParam) {
      ...
      fetchCourses(debouncedSearchQuery, currentPage, limit, selectedDepartmentId, selectedSchoolId) // Refresh courses list
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation
    const schoolIdToUse = isEditing ? (courseToEdit as any)?.schoolId?._id || formData.schoolId : (selectedSchoolIdForForm || formData.schoolId)
    if (!formData.courseName || !formData.courseCode || formData.departmentIds.length === 0 || !schoolIdToUse) {
      toast.error("Please fill all required fields including school")
      return
    }
    ...
    try {
      ...
      setIsModalOpen(false)
      resetForm()
      fetchCourses(debouncedSearchQuery, currentPage, limit, selectedDepartmentId, selectedSchoolId)
    } catch (error: any) {
      if (error.status === 409) {
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      ...
      <div className="mb-6 mt-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-3 items-center w-full">
          <div className="relative w-full">
            <Label className="mb-1 block text-sm">Search Courses</Label>
            <Input
              type="text"
          ...
          <div className="min-w-[200px] w-full">
            <Label className="mb-1 block text-sm">Filter by School</Label>
            <Select
      ...
      <Card className="shadow-sm">
                ...
                {isLoading ? (
                  ...
                  courses.map((c) => (
                    <TableRow key={c._id}>
                      ...
                      <TableCell className="hidden md:table-cell">{(c as any).schoolId?.name || "—"}</TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="sm" title="View" onClick={() => router.push(`/super-admin/courses/view/${c._id}?schoolId=${(c as any).schoolId?._id || ""}`)}><Eye className="h-4 w-4" /></Button>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t">
          <div className="flex items-center gap-2">
            ...
            <Select value={String(limit)} onValueChange={(v) => { const l = Number(v); setLimit(l); setCurrentPage(1); fetchCourses(debouncedSearchQuery, 1, l, selectedDepartmentId, selectedSchoolId) }}>
              ...
              <div className="grid gap-6 py-4">
                <div className="space-y-2">
                  ...
                  {isSchoolsLoading ? (
                    ...
                    <Input
                      value={(courseToEdit as any)?.schoolId?.name || ""}
                      disabled
                      className="border-gray-300 bg-gray-50"
                    />
                  ) : (
                  <Select
                    value={selectedSchoolIdForForm || formData.schoolId || ""}
                    onValueChange={async (value) => {
                      setSelectedSchoolIdForForm(value)
                      setFormData((prev) => ({ ...prev, schoolId: value }))
                      await fetchDepartments(value)
                      // Clear any previously selected departments if switching school
