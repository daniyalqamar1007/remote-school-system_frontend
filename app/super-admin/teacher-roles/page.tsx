...
export default function SuperAdminTeacherRolesPage() {
  ...
  const fetchTeachers = async (page = 1, limit = 10, search = '', schoolId = 'all') => {
    try {
      setLoading(true)
      const token = localStorage.getItem('token') || localStorage.getItem('accessToken')
  ...
  const handlePageChange = (newPage: number) => {
    ...
    fetchTeachers(newPage, pageSize, searchTerm, selectedSchoolId)
  }

  const handlePageSizeChange = (newSize: number) => {
    setPageSize(newSize)
    setCurrentPage(1)
    fetchTeachers(1, newSize, searchTerm, selectedSchoolId)
  }
  ...
  const handleSchoolFilterChange = (schoolId: string) => {
    ...
    setCurrentPage(1)
    const actualSchoolId = schoolId === 'all' ? '' : schoolId
    fetchTeachers(1, pageSize, searchTerm, actualSchoolId)
  }

  // Handle search with debounce
  const debouncedSearch = useCallback(
    debounce((search: string, schoolId: string) => {
      setCurrentPage(1)
      fetchTeachers(1, pageSize, search, schoolId)
    }, 500),
    [pageSize]
  )
  ...
  const handleSearchChange = (value: string) => {
    setSearchTerm(value)
    debouncedSearch(value, selectedSchoolId)
  }
  ...
  const handleToggleChange = async (
    ...
    try {
      ...
      // Get teacher's schoolId for the API call
      const teacher = teachers.find(t => t._id === teacherId)
      const teacherSchoolId = teacher?.schoolId?._id || ''

      const params: any = {}
      if (teacherSchoolId) {
        params.schoolId = teacherSchoolId
      }
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <Select value={selectedSchoolId} onValueChange={handleSchoolFilterChange}>
                  <SelectTrigger>
                    ...
                    teachers.map((teacher) => (
                      <TableRow key={teacher._id}>
                        ...
                        <TableCell className="hidden lg:table-cell">
                          <Badge variant="secondary">{teacher.schoolId?.name || 'N/A'}</Badge>
                        </TableCell>
