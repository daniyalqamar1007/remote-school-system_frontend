    ...
    try {
      ...
      if (response) {
        if (response.success && response.data) {
          programsList = Array.isArray(response.data) ? response.data : (Array.isArray(response.data.programs) ? response.data.programs : [])
        } else if (Array.isArray(response.data)) {
          programsList = response.data
        } else if (Array.isArray(response.programs)) {
          programsList = response.programs
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 pt-4 border-t">
              <Link href="/teacher/sports/schedule" className="flex-1">
